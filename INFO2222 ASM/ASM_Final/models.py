...
from sqlalchemy import create_engine, Column, Integer, String, ForeignKey, Table, Enum, UniqueConstraint, Text
...
class User(Base):
    ...
    status: Mapped[str] = mapped_column(
        Enum('on-line', 'offline', 'disconnection', 'chatting', name='user_status', create_type=False),
        default='offline')
    role: Mapped[str] = mapped_column(Enum('student', 'staff', 'admin', 'guest', name='user_roles', create_type=False),
                                      default='student')
    ...
    # Define buddy relationships between users (assuming you have an association table already defined)
...
class FriendRequest(Base):
    ...
    status = Column(Enum('pending', 'accepted', 'rejected', 'blocked', name='request_status'))
