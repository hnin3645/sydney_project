...
def connect():
    ...
    room_id = request.cookies.get(f"{username}_room_id")

    if username is None:
        print("Connect event without username.")
        return

    if room_id is None:
        update_user_status(username, 'on-line')
        return
    ...
    update_user_status(username, 'chatting')
...
def disconnect():
    ...
    room_id = request.cookies.get(f"{username}_room_id")
    if username is None:
        print("Disconnect event without username.")
        return

    if room_id is None:
        update_user_status(username, 'offline')
        print(f"{username} marked as offline due to missing room ID on disconnect.")
        return

    update_user_status(username, 'disconnection')
...
def join(sender_name, receiver_name):
    ...
    update_user_status(sender_name, 'chatting')
